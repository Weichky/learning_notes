...
固熔体的特点：
- 固熔体具有溶剂组元的点阵类型，晶格常数稍有变化.
- 有一定的成分范围，也就是说，组元的含量可在一定范围内改变而不会改变固熔体点阵类型的改变.由于固熔体的成分范围是可变的，而且有一个溶解度极限，故通常固熔体不能用一个化学式表示.
- 金属固熔体具有比较明显的金属性质，固熔体中的结合键主要是金属键.
固熔体分类：
- 置换固熔体
- 间隙固熔体
...
- 有序固熔体`超结构、超点阵，有规律的占据溶剂结构中的固定位置，而且溶剂与溶质原子之比一定.`
- 无序固熔体

有限置换固熔体*Cu-Zn、Cu-Ti、Fe-W、Fe-Mo*
无限置换固熔体*Ni-Cu、Au-Ag、Fe-Cr、Fe-Ni、Fe-Co、Fe-V*

若组元晶体结构不同，晶格与固熔体相同的组元为溶剂；
若结构相同，则摩尔分数大于50%的组元是溶剂.

绝大部分金属元素之间都能形成置换固熔体，但是只有少部分金属元素之间可以形成无限固熔体
- 只有0价或1价的元素相互溶解有可能形成无限固熔体
![化合价](./images/img3.jpeg "元素化合价")
- $\alpha-Fe$无限固溶V、Cr
- $\gamma-Fe$无限固溶Mn、Co、Ni

### 金属间化合物
有限固溶体超过固溶度后。由金属与金属或金属与类金属（也称为半金属、准金属）（B、Si、Ge、As、Sb、Te、Po、At`、Se、A、C`）之间发生化学相互作用形成的具有金属基本特性而不同于其组元的长程有序晶体结构化合物称为金属间化合物.

成分在二元相图上所处的位置总是位于两个端际固溶体之间的中间部位，即处于两种固溶体的极限溶解度之间的，称为**中间相**.
- 金属间化合物
- 有序固溶体

- 金属间化合物通常按一定或大致一定的原子比组成，可以用化学分子式来表示.
- 除正常价化合物外，大多数金属间化合物的分子式不遵循化合价规律.
- 是一个新相.
- 可溶入化学性质相近的其他元素，形成以化合物为基的固溶体，其化学组成可在一定范围内变化.
...
## 相图
### 相、相平衡及相图制作
**相** 在一个系统中，具有同一聚集状态的均匀部分称为**相**.
**相平衡** 的热力学条件要求每个组元在各相中的化学位($\mu$)必须相同.从动力学上，这种相平衡是一种动态平衡.$$f=c-p+2$$
在恒压条件下$$f=c-p+1$$
式中f为自由度数，c为组成材料系统的组元数，p为平衡相的数目.

Clausius-Clapeyron 方程$$\frac{{\rm d}p}{{\rm d}T}=\frac{\Delta H}{T\Delta V_m}$$
$其中\Delta H为相变潜热，\Delta V_m为摩尔体积变化，T为两相平衡温度.$

$w表示质量分数，x表示摩尔分数.$

由液相直接结晶出单相固熔体的过程，称为**匀晶转变**.
完全具有匀晶转变的相图，称为**匀晶相图**.
合金从液态无限缓慢地冷却，原子扩散非常充分，时时达到相平衡的凝固方式，称为**平衡凝固**.

固熔体合金凝固时结晶出来的固相成分与原液成分不同，固熔体凝固形核时，除了需要能量起伏和结构起伏外，还需要成分起伏，因而固熔体凝固形核比纯金属困难.
固熔体凝固需要一定的温度范围，在此温度范围的每一温度下，只能凝固出来一定数量的固相，即固熔体凝固必须依赖于异类原子的相互扩散.这需要时间，所以凝固速率比纯金属慢.
