...
### 梁的变形，挠曲线微分方程及其积分
**假设**
平面假设`变形前原为平面的梁的横截面变形后仍保持平面，且垂直于变形后的梁轴线`
单向受力假设`只发生轴向拉压，而纤维之间无挤压`

纵向纤维的应变与它到中性层的距离成正比.$$\begin{aligned}\varepsilon=&\frac{y}{\rho}\\\sigma=E\varepsilon=&E\cdot\frac{y}{\rho}\end{aligned}$$
其中$y为该纤维到中性层的距离$.
中性面上各点应力为0.

$EI_z称为梁的抗弯刚度$$$\frac{1}{\rho}=\frac{M}{EI_z}$$
$$\sigma=\frac{My}{I_z}$$
其中$I_z$称为横截面对中性轴的惯性矩.式用于求正应力的数值，M和y均取绝对值.可以不注明应力的正负号，只在计算结果后用括号注明“拉”或“压”.